...
def generate_id_card(name, branch, dob, blood_group, mobile_no, address, profile_image):
    ...
    # Add ID number
    draw.text((20, 20), f"ID: {id_number}", fill="white")
    
    # Add user details with colors
    draw.text((20, 50), f"Name:", fill="blue")
    draw.text((150, 50), f"{name}", fill="black")
    draw.text((20, 80), f"Branch:", fill="blue")
    draw.text((150, 80), f"{branch}", fill="black")
    draw.text((20, 110), f"DOB:", fill="blue")
    draw.text((150, 110), f"{dob}", fill="black")
    draw.text((20, 140), f"Blood Group:", fill="blue")
    draw.text((150, 140), f"{blood_group}", fill="black")
    draw.text((20, 170), f"Mobile No:", fill="blue")
    draw.text((150, 170), f"{mobile_no}", fill="black")
    draw.text((20, 200), f"Address:", fill="blue")
...
def validate_input(name, branch, dob, blood_group, mobile_no, address, profile_image):
    # Perform validation here
    if not name or not branch or not dob or not blood_group or not mobile_no or not address:
...
def save_to_csv(id_number, name, branch, dob, blood_group, mobile_no, address):
    with open('data.csv', 'a') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([id_number, name, branch, dob, blood_group, mobile_no, address, datetime.now()])
...
def main():
    ...
    if st.button("Generate ID Card"):
        if validate_input(name, branch, dob, blood_group, mobile_no, address, profile_image):
            ...
            # Generate ID card
            id_card, id_number = generate_id_card(name, branch, formatted_dob, blood_group, mobile_no, address, profile_image_pil)
            st.image(id_card, caption=f"ID Card for {name}", use_column_width=True)
            
            # Save data to CSV
            save_to_csv(id_number, name, branch, formatted_dob, blood_group, mobile_no, address)
            
            # Download link for ID card image
            st.markdown(f"Download your ID card [here](data:image/png;base64,{id_number}).")
    
    # Additional features can be added here, such as email notifications, export options, etc.
